...
`addEventListener` and `removeEventListener` identify listeners by *reference*, not by code. Two functions with the same body are still two different function objects — they have different identities in memory.
...
Memory consequence: an event listener you can't remove is a *memory leak*. The element, the function, and everything the function closes over stay in memory until the element itself is destroyed. In a long-lived page, leaks accumulate.
