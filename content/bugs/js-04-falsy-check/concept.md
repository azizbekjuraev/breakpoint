...
The shortcut `if (!value)` treats *all* falsy values as "missing" — but most of the time, what you actually mean is "missing" (null or undefined), not "any falsy value." When `0`, `''`, or `false` are valid inputs, the truthy check rejects them as if they were missing.
...
- `value == null` — true for `null` *and* `undefined` only. This is the rare case where `==` is idiomatic.
