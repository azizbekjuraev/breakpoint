...
React identifies which state belongs to which `useState` call by _call order_. There's no name, no tag — just the position. On every render, React expects to see exactly the same sequence of hook calls.
...
The fix is structural: move every hook to the top of the function, before any conditional logic. If you need conditional behavior, conditionally use the _value_, not the hook.
